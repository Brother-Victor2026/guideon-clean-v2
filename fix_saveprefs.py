...
old_prefs = '''function savePreferences() {
  const lang = document.getElementById('langSelect')?.value || 'auto';
  const theme = document.getElementById('themeSelect')?.value || 'dark';
  const length = document.getElementById('lengthSelect')?.value || 'normal';                                                            localStorage.setItem('gprefs', JSON.stringify({lang, theme, length, timestamp: Date.now()}));
...
print("✅ savePreferences() corrigée")
